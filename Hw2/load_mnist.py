...
def mnist(noTrSamples=1000, noTsSamples=100, \
    ...
    #loading all the data into trdata,trlabels, tsdata, tslabels (Check 16 vs 8)
    ...
    #standardize between 0 and 1 by dividiing by 255
    trData = trData/255.
    tsData = tsData/255.

    #initializing the training and test matrices to 0
    tsX = np.zeros((noTsSamples, 28*28))
    trX = np.zeros((noTrSamples, 28*28))
    tsY = np.zeros(noTsSamples)
    trY = np.zeros(noTrSamples)

    #tsX - 1000*784, tsY - 1000*1
    count = 0
...
def main():
    ...
    #print(trX[:,5])
    #print(trX.shape)
    #print(trX[:,5].reshape(28, -1).shape) # reshaping to 28x28 matrix( decomposing feature vector into matrix to plot) 
